...
export default function PengumumanAdminPage() {
  ...
  const fetchAnnouncements = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/pengumuman?${params.toString()}`);
      if (response.ok) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/admin/pengumuman/${deleteId}`, {
        method: 'DELETE',
      });
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      const response = await fetch('/api/admin/pengumuman/bulk-delete', {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        toast.success(`${selectedIds.length} pengumuman berhasil dihapus`);
        fetchAnnouncements();
      } else {
  ...
  const ActionButtons = ({ item }: { item: Announcement }) => (
    <TooltipProvider>
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <Button asChild className="bg-[#1B99F4] hover:bg-[#1B99F4]/90 w-full sm:w-auto">
            <Link href="/admin/pengumuman/tambah" className="gap-2">
                  ...
                  <div className="flex gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        ...
                        {item.expireAt && ` • Berakhir: ${formatDate(item.expireAt)}`}
                      </p>
                    </div>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="w-[50px]">Prioritas</TableHead>
                        ...
                        <div>
                          ...
                          {item.expireAt && (
                            <p className="text-xs text-muted-foreground">
                              Berakhir: {formatDate(item.expireAt)}
                            </p>
                          )}
                        </div>
                      </TableCell>
        ...
        <DeleteConfirmationDialog
          ...
          description="Pengumuman akan dihapus secara permanen dan tidak dapat dikembalikan."
        ...
        <DeleteConfirmationDialog
          ...
          title={`Hapus ${selectedIds.length} Pengumuman?`}
          description="Pengumuman yang dipilih akan dihapus secara permanen dan tidak dapat dikembalikan."
