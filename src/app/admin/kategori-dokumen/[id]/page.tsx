...
export default function EditKategoriDokumenPage({ params }: PageProps) {
  ...
  const fetchCategory = async () => {
    try {
      const response = await fetch(`/api/admin/kategori-dokumen/${resolvedParams.id}`);
      if (response.ok) {
        ...
        router.push('/admin/kategori-dokumen');
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = isNew
        ? '/api/admin/kategori-dokumen'
        : `/api/admin/kategori-dokumen/${resolvedParams.id}`;
      ...
      <div className="space-y-6 max-w-2xl mx-auto">
        ...
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" asChild>
            <Link href="/admin/kategori-dokumen">
              <ArrowLeft className="h-5 w-5" />
            </Link>
          </Button>
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-foreground">
              {isNew ? 'Tambah Kategori' : 'Edit Kategori'}
            </h1>
            <p className="text-muted-foreground mt-1">
              {isNew ? 'Buat kategori dokumen baru' : 'Perbarui informasi kategori'}
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                />
                <p className="text-xs text-muted-foreground">
                  Nama icon dari Lucide React (contoh: FileText, FolderOpen)
                </p>
              </div>
          ...
          <div className="flex justify-end gap-4">
            <Button type="button" variant="outline" asChild>
              <Link href="/admin/kategori-dokumen">Batal</Link>
            </Button>
