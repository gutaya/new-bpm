...
export default function KategoriAkreditasiAdminPage() {
  ...
  const fetchCategories = async () => {
    try {
      const response = await fetch('/api/admin/kategori-akreditasi');
      if (response.ok) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/admin/kategori-akreditasi/${deleteId}`, {
        method: 'DELETE',
      });
  ...
  const handleToggleActive = async (id: string, currentStatus: boolean) => {
    try {
      const response = await fetch(`/api/admin/kategori-akreditasi/${id}`, {
        method: 'PATCH',
  ...
  const ActionButtons = ({ item }: { item: AccreditationCategory }) => (
    <TooltipProvider>
      <div className="flex items-center justify-end gap-0.5 sm:gap-1">
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            ...
            >
              <Link href={`/admin/kategori-akreditasi/${item.id}`}>
        ...
        {/* Delete Button - Only show if no accreditations */}
        {(!item._count || item._count.accreditations === 0) && (
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                variant="ghost"
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm sm:text-base text-muted-foreground mt-1">
              Kelola kategori untuk data akreditasi
            </p>
          </div>
          <Button asChild className="bg-[#1B99F4] hover:bg-[#1B99F4]/90 w-full sm:w-auto">
            <Link href="/admin/kategori-akreditasi/tambah" className="gap-2">
              ...
              <Card key={item.id} className="p-3 sm:p-4">
                <div className="flex gap-3">
                  ...
                  <div className="w-12 h-12 rounded-lg bg-[#1B99F4]/10 flex items-center justify-center shrink-0">
                    <Award className="h-6 w-6 text-[#1B99F4]" />
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-2">
                      ...
                      <Badge variant="outline" className="text-xs gap-1">
                        ...
                        {item._count?.accreditations || 0} akreditasi
                      </Badge>
                    </div>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="w-[100px]">Akreditasi</TableHead>
        ...
        <DeleteConfirmationDialog
          ...
          description="Kategori akan dihapus secara permanen. Kategori yang memiliki data akreditasi tidak dapat dihapus."
