...
export default function EditGaleriPage({ params }: PageProps) {
  ...
  const fetchImage = async () => {
    try {
      const response = await fetch(`/api/admin/galeri/${resolvedParams.id}`);
      if (response.ok) {
      ...
      toast.error('Gagal memuat data gambar');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.title.trim()) {
      toast.error('Judul gambar wajib diisi');
      return;
    }
    ...
    try {
      const url = isNew
        ? '/api/admin/galeri'
        : `/api/admin/galeri/${resolvedParams.id}`;
      ...
      if (response.ok) {
        toast.success(isNew ? 'Gambar berhasil ditambahkan' : 'Gambar berhasil diperbarui');
        router.push('/admin/galeri');
      } else {
    ...
    <AdminLayout>
      <div className="space-y-6 max-w-4xl mx-auto">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-foreground">
              {isNew ? 'Tambah Gambar' : 'Edit Gambar'}
            </h1>
            <p className="text-muted-foreground mt-1">
              {isNew ? 'Tambahkan gambar baru ke galeri' : 'Perbarui informasi gambar'}
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">Informasi Gambar</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setFormData((prev) => ({ ...prev, albumId: value === 'none' ? '' : value }))
                  }
                >
                  <SelectTrigger>
              ...
              <div className="flex items-center justify-between">
                <div className="space-y-0.5">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Aktifkan untuk menampilkan gambar di galeri
                  </p>
                </div>
                <Switch
                  id="isActive"
